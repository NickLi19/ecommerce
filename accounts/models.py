...
from ecommerce.utils import unique_order_key_generator
...
class EmailActivationManager(models.Manager):
    ...
    def email_exists(self, email):
        return self.get_queryset().filter(Q(email=email) | Q(user__email=email)).filter(activeted=False)


class EmailActivation(models.Model):
    user = models.ForeignKey(User, related_name='email_activation', on_delete=models.CASCADE)
    ...
    def send_activation(self):
        if not self.activated and not self.forced_expired:
            if self.key:
                ...
                txt_ = get_template("registration/emails/verify").render(context)
...
def pre_save_email_activation(sender, instance, *args, **kwargs):
    if not instance.activated and not instance.forced_expired:
        if not instance.key:
            instance.key = unique_order_key_generator(instance)
