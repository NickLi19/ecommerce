...
class ObjectViewed(models.Model):
    user = models.ForeignKey(User, blank=True, null=True, related_name='object_viewed', on_delete=models.CASCADE)
...
def object_viewed_receiver(sender, instance, request, *args, **kwargs):
    ...
    if request.user.is_authenticated:
        user = request.user
    new_view_obj = ObjectViewed.objects.create(user=user, content_type=c_type,
                                               object_id=instance.id, ip_address=get_client_ip(request))
...
def post_save_user_changed_receiver(sender, instance, created, *args, **kwargs):
    if not created:
        if instance.is_active == False:
            qs = UserSession.objects.filter(instance.user, ended=False, active=False)
            for i in qs:
                i.end_session()
